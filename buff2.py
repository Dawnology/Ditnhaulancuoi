...
\033[1;31m[\033[1;37m</>\033[1;31m]  \033[1;35mADMIN: \033[1;36mCheapluxury
\033[1;31m[\033[1;37m</>\033[1;31m]  \033[1;31mTelegram: \033[1;33mhttps://t.me/cheapluxurychat
...
def send(__device_id, __install_id, cdid, openudid, soluong):
    ...
    for x in range(soluong):
        try:
            ...
            response = r.post(
                ...
                verify=False,
                proxies={"http": proxy_format + proxy, "https": proxy_format + proxy} if config['proxy'][
                    'use-proxy'] else {}
            )
            reqs += 1
            try:
                if response.json()['status_code'] == 0:
                    _lock.acquire()
                    print(
                        f'{do}[{vang}VM{do}] {do}[{xanh_la}BUFF VIEW SUCCESS{do}] {do}{xanh_duong}+: {trang}{success}{do} View')
                    success += 1
                    _lock.release()
            except:
                if _lock.locked():
...
if __name__ == "__main__":
    ...
    print(banner)
    try:
        link = input(f'{do}[{trang}{kt_code}{do}] {xanh_la}INPUT ID VIDEO TIKTOK: {trang}')
        soluong = input(f'{do}[{trang}{kt_code}{do}] {xanh_la}THREAD AMOUNT(MUST BE LOWER 200): {trang}')
        if not soluong:
            soluong = 50   
        soluong = str(soluong)
        print(f'{do}[{trang}{kt_code}{do}] {xanh_la}THREADS: {soluong} {trang}') 
        print(f'{trang}- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -')
        __aweme_id = str(
    ...
    while True:
        ...
        if eval(base64.b64decode("dGhyZWFkaW5nLmFjdGl2ZV9jb3VudCgpIDwgMjAw")):
            did, iid, cdid, openudid = device.split(':')
            eval(base64.b64decode("dGhyZWFkaW5nLlRocmVhZCh0YXJnZXQ9c2VuZCxhcmdzPVtkaWQsaWlkLGNkaWQsb3BlbnVkaWQsc29sdW9uZ10pLnN0YXJ0KCk="))